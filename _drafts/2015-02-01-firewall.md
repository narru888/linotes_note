---
toc: true
toc_label: "Linux 的使用 - 防火墙"
toc_icon: "laptop"
title: "Linux 的使用 - 防火墙"
tags: linux iptables firewalld 防火墙
categories: "tools"
classes: wide
excerpt: ""
header:
...
iptables 是一个 **用户空间** 的程序，通过维护特定的表格和规则链来管理防火墙。针对不同的协议，会使用不同的内核模块。如 IPv4 使用 iptables，IPv6 使用 ip6tables，ARP 使用 arptables，以太帧使用 ebtables。

iptables 需要 root 权限来运行。通常安装于 `/usr/sbin/iptables` 目录。

`iptables` 一词也经常代指该内核级的防火墙。iptables  可以直接配置，也可以通过前端和图形界面来配置。

iptables 的继任者为 nftables，在内核版本 3.13 之后合并到 mainline。

iptables 不是真正的防火墙，它仅仅是一个客户端代理，用户通过它把防火墙的配置执行到 “安全框架” 中，这个安全框架才是真正的防火墙，名为 **netfilter**，位于 **内核空间**。
{: .notice}

netfilter/iptables 组成了 Linux 平台的包过滤防火墙，可以完成封包过滤、封包重定向和网络地址转换等功能。netfilter/iptables 简称 iptables。

netfilter 是 Linux 核心层内部的一个数据包处理模块，有如下功能：
