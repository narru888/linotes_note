...
* 网络设备接口必须支持多个 MAC 地址
* 如果应用程序可以使用网桥（在一台 docker 宿主机上）或叠加网络（穿越多个 docker 宿主机进行通信），则长期来说，使用 macvlan 网络比较理想
...
创建 macvlan 网络时，可以用网桥模式，也可以用 802.1q trunk 网桥模式。

* 在网桥模式中：macvlan 的流量会走主机的物理设备
* 在 802.1 q trunk 网桥模式中：流量会走 802.1q 子接口，这些接口是由 docker 在线生成的。通过这些子接口，可以更加精细地控制路由和过滤。
