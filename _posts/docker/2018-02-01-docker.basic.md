...
`-p`：端口映射。容器的 3000 端口映射到本机的 8000 端口。docker 中运行的进程，其接触到的文件系统和网络接口都是虚拟的，与本机的文件系统和网络接口是隔离的，因此需要定义容器与物理机的端口映射（map）。
...
命令 | 说明
--- | ---
查看镜像列表，隐藏无名称无标签的  |  `docker image ls`
查看所有镜像列表  |  `docker image ls -a`
构建镜像  |  `docker build -t imagename .`
删除镜像文件  |  `docker image rm <image id>`
删除所有镜像文件  |  `docker image rm $(docker image ls -aq)`
登陆 docker hub  |  `docker login`
为本地镜像打标签  |  `docker tag <image> username/repo:tag`
上传打过标签的镜像  |  `docker push username/repo:tag`
...
命令 | 说明
--- | ---
启动容器  |  `docker run username/repo:tag`
启动容器，端口映射  |  `docker run -p 4000:80 imagename`
启动容器，后台运行  |  `docker run -d -p 4000:80 imagename`
