...
在网络技术中，bridge，即网桥，是链路层的设备，用来在不同的网段间转发流量。网桥可以是硬件设备，也可以是运行于宿主机内核中的软件。

在 docker 中，网桥是使用软件实现的。该网桥使得连接到同一网桥的容器可以互相通信，同时还会将其与那些没有连接到此网桥的容器相互隔离。Docker 的网桥驱动会在宿主机中自动安装规则，这样，连接到不同网桥的容器之间就无法直接通信了。
