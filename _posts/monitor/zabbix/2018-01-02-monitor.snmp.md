...
由管理端发给受控端，请求同时进行多个 GetNextRequest 的迭代。它是 GetNextRequest 的优化版本。在返回的应答消息中包含多对变量/值。可以使用 PUD 相关的 `non-repeaters` 和 `max-repetitions` 字段，以控制应答的具体行为。由 SNMPv2 引入。
