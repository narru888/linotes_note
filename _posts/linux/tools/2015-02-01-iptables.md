...
64 位系统的虚拟地址空间有 **256TB**，内核态能用 **一半**，只需要关心物理内存的使用情况。
...
* 64 位系统，8G 内存的主机，按默认 `CONNTRACK_MAX` 为 262144，`HASHSIZE` 为 65536

其 conntrack 内存占用量为：

`262144 * 352 + 65536 * 8 = 92798976 = 88.5 MB`

* 如果企业内存量足够大：`CONNTRACK_MAX` 为 1048576，`HASHSIZE` 为 262144
