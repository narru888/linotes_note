...
nf_conntrack_count / nf_conntrack_buckets
...
基于该因素的考虑，建议当 `nf_conntrack_count` 的值持续超过 `nf_conntrack_max` 的 **20%** 时，就该考虑 **扩容** 了。因为 bucket 的值默认是 max 的 25%，用了 max 的 20% 也就是 80% 的桶都有元素了（假设没冲突）。
{: .notice}
