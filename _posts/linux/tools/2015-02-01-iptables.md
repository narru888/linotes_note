...
内核使用 nf_conntrack 模块来追踪 iptables 网络包的状态，连接追踪引擎会使用一个 **哈希表** 来保存追踪的元组。每个连接会添加两次，一次正向，一次反向。NAT 就是依据这一正一反的元组来工作的。

哈希表的关键元素为关联列表，一个追踪条目只是该列表中的某个节点，这样的关联列表有多个。因此，哈希表的每个条目中（也称 bucket）都包含了一系列相互关联的追踪条目。

So each hash table entry (also called a bucket) contains a linked list of conntrack entries. To access a conntrack entry corresponding to a packet, the kernel has to:
