...
哈希表的关键元素为关联列表，一个追踪条目只是该列表中的某个节点，这样的关联列表有多个。因此，哈希表的每个条目中（也称 bucket）都包含了一系列相互关联的追踪条目。要想访问某个数据包对应的追踪条目，内核必须：

* 根据数据包的特点出计算一个哈希值，该值在哈希表中会用做索引，与该索引一起保存了该数据包的若干个追踪条目。
* 
