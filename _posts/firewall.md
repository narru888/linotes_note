...
CentOS 内置了一个很强大的防火墙，通常称为 iptabes，但更准确地说是 iptables/netfilter。

`Iptables` 是 **用户空间的模块**，在命令行，用户可以向预定义的表中增加防火墙规则。

`Netfilter` 是一个 **内核模块**，内建于内核中，真正执行过滤的是它。

有许多图形界面的 iptables 前端程序，用户可以用来添加或修改规则，但它们通常缺少命令行的灵活性，让用户无法充分了解当下发生了什么。

iptables 使用 “IP 地址、协议、端口” 这三个概念。

iptables 会用向预定义的规则链 `INPUT`、`OUTPUT`、`FORWARD` 中加入规则，这些规则链被用来检查网络流量（IP 包），每种规则链对于流量的去向需要有各自的决定，是放行还是丢弃数据包。这些行为被称为目标，targets，最常见的两个预定义的目标是 DROP 和 ACCEPT，丢弃和接受。

使用 iptables 需要超级用户权限。
