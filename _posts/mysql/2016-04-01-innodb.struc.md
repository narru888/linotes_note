...
* **校验码**：32 位的校验码保存在文件头中，文件尾中还保存了一个 32 位的旧格式的校验码。旧的校验可以抛弃，该空间在某一时刻会被回收。
* **指针**：指向逻辑前一页、后一页的指针，保存于文件头。借此可以生成双链接的页面列表，指针可在索引页中用来链接同一级别的所有页面，使得全索引扫描更加高效。许多页面类型不使用这些字段。
