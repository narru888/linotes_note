...
每个空间都被划分为多个页面，通常每个页面的大小为 16 KB。空间中的每个页面都被分配给一个 32 位的 **页面编号**，经常被称为 **偏移量**，即从空间起始位置算起，该页面的偏移量。
...
每个页面都有一个 38 字节的 FIL Header，即 File Header，**页头**； 以及一个 8 字节的 FIL Trailer，即 File Trailer，**页尾**。
