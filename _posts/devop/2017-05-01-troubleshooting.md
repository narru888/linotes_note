...
🚩 如果第一个日志中包含 `CREATE TEMORARY TABLE` 语句，而第二个日志包含要使用该临时表格的语句，这种用不同的连接来处理二进制日志的方法就会产生问题。当第一个 mysql 进程结束时，服务器会删除临时表格，于是第二个 mysql 进程是找不到它的，服务器会报错 `unknown table`。

要想避免这类问题，一定要使用 **单一** 的连接来执行所有的二进制日志内容：

```bash
shell> mysqlbinlog binlog.000001 binlog.000002 | mysql -u root -p
```

或者提前把多个日志文件合并为一个，再执行：
