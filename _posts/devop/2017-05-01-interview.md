...
1. 客户端向本机配置的 **本地域名服务器** 发出 DNS 查询请求。
2. 本地域名服务器收到请求后，先查询本地的 **缓存**
	- 如果有该域名的记录项，则直接把查询的结果返回给客户端
	- 如果没有该域名的记录，本地域名服务器则以 DNS 客户端的角色，向 **根域名服务器** 发送 DNS 查询请求
3. 根域名服务器收到 DNS 请求后，把查询得到的 DNS 域名所对应的顶级域名服务器地址返回给本地域名服务器。
4. 本地域名服务器根据返回的地址，向对应的 **顶级域名服务器** 发送 DNS 查询请求。
5. 顶级域名服务器收到查询请求后，先查询自己的 **缓存**
	- 如果有该域名的记录项，则将其返回给本地域名服务器，本地域名服务器继而返回给 DNS 客户端
	- 如果没有，则向本地域名服务器返回该域名所对应的 **二级域名服务器** 地址。
6. 本地域名服务器按照上述方法遍历三级、四级域名服务器查询，直到 **权威域名服务器** 返回最终的记录给本地域名服务器。然后再由本地域名服务器返回给 DNS 客户端，同时，本地域名服务器会缓存本次查询得到的记录项。
