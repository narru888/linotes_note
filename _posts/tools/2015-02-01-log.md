...
介于 prerotate 和 endscript 之间的各行命令，会在滚动开始之前执行，如修改日志属性。对于已加上隐藏属性的文件特别必要。只能用于特定日志文件的定义中。
...
介于 postrotate 和 endscript 之间的各行命令，会在滚动完成之后执行，如重启某服务。对于已加上隐藏属性的文件特别必要。只能用于特定日志文件的定义中。
