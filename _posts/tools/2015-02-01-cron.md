...
crontab 在运行任务计划时，会以该用户的身份运行，并且 **工作目录** 默认为 $HOME，而且看不到 bash 的 **环境变量**。因此，在做任务计划脚本时，一定要充分考虑到这些因素。通常有必要使用 `source` 加载正确的配置文件，必要时还需要显式指定家目录，如 `HOME=/home/neo`，否则容易发生 `Permission denied (publickey)` 的问题。
{: .notice--success}
