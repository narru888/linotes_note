...
在 cron 中运行 git 时，可能会遇到 `Permission denied (publickey).` 的问题，这是因为 SSH 客户端能看到和使用的一些参数，cron 是看不到的，比如 SSH 公钥。因此在 cron 环境中连接 SSH 时是没有公钥可用的，必须手动指定。通常借助 keychain 把公钥先放到缓存中，再导入相关的环境变量就可以解决。
