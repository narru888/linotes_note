...
在 cron 中运行 git 时，可能会遇到 `Permission denied (publickey).` 的问题，这是因为 SSH 客户端能看到和使用的一些参数，cron 是看不到的，比如 SSH 公钥和私钥。因此在 cron 环境中连接 SSH 时是没有私钥可用的，必须手动指定。

SSH 协议会使用 **转发客户端** 机制，即 agent forwarding。SSH 客户端允许 SSH 服务器使用其本地的 ssh-agent 客户端程序，感觉就像该客户端是从服务器本地连接进来的。此时，远端的客户端程序会先与 **服务器端的客户端程序** 通信，服务器与远端客户端的通信都会通过它进行 **中转**。
