...
数据库密码以明文方式存储在 vCS/vCSA 所在的 Windows 虚拟机或物理主机中。包含密码的文件通过使用操作系统保护进行保护，也就是说，您必须是 Windows 本地管理员或 Linux root 用户才能访问和读取这些文件。
...
您不能在安装了 vCS 之后将其指向旧的外部 vCS 数据库。只能通过升级连接至旧 vCS 数据库的 vCS 实例，将该数据库升级到最新版本。
