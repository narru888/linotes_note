...
不是 vCenter Server 和 vCenter Server Appliance 所支持数据库的完整列表。有关 vCenter Server 支持的特定数据库版本和服务包配置的信息，请参见 VMware 产品互操作性列表。vCenter Server Appliance 支持的 Oracle 数据库版本与 vCenter Server 相同。在1 中仅提供未在产品互操作性列表中列出的特定数据库配置说明。
