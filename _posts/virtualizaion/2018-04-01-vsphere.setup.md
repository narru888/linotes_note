...
为确保不丢失任何数据，请在安装 ESXi 前将数据迁移至其他计算机。
{: .notice}

如果在包含 ESXi 或 ESX 的先前安装或 VMFS 数据存储的磁盘上安装 ESXi，安装程序会为您提供进行升级的选项。请参见《vSphere 升级》文档。
...
将 ESXi 安装到软件 iSCSI 磁盘上时，必须配置目标 iSCSI 限定名 (IQN)。
